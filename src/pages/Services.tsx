...
const services = [
  {
    ...
    features: [
      ...
      "In-depth market analysis and pricing insights",
      "Expert negotiation to secure the best deal",
      "Coordination with inspectors, lenders, and attorneys",
  ...
  },
  {
    ...
    features: [
      ...
      "Targeted marketing across premium channels",
      "Open house coordination and private showings",
      "Expert negotiation and smooth closing support",
    ],
  },
  {
    id: "rentals",
    ...
    features: [
      "Residential homes and apartment rentals",
      ...
      "Ideal for professionals, families, and relocating individuals",
    ],
  },
  {
    id: "renovations",
    ...
    features: [
      "Renovation planning and consultation",
      "ROI-focused improvement recommendations",
      "Trusted contractor network and coordination",
      "Pre-sale renovation strategies",
  ...
  },
  {
    ...
    features: [
      ...
      "No-obligation valuation report",
    ],
  },
];
    ...
    <Layout>
      ...
      {services.map((service, index) => (
        <section
          key={service.id}
          id={service.id}
          className={`section-padding ${index % 2 === 0 ? "bg-background-secondary" : "bg-background"}`}
        >
          <div className="container-custom">
            <div className={`grid lg:grid-cols-2 gap-12 lg:gap-16 items-center ${index % 2 === 1 ? "lg:flex-row-reverse" : ""}`}>
              <div className={index % 2 === 1 ? "lg:order-2" : ""}>
                <div className="p-3 bg-secondary rounded-xl w-fit mb-4 animate-fade-up opacity-0" style={{ animationDelay: "100ms" }}>
                  <service.icon className="w-8 h-8 text-foreground" />
                </div>
                <span className="text-sm text-muted-foreground uppercase tracking-wider mb-2 block animate-fade-up opacity-0" style={{ animationDelay: "150ms" }}>
                  {service.subtitle}
                </span>
                <h2 className="font-serif text-3xl md:text-4xl font-semibold text-foreground mb-4 animate-fade-up opacity-0" style={{ animationDelay: "200ms" }}>
                  {service.title}
                </h2>
                <p className="text-muted-foreground text-lg mb-6 animate-fade-up opacity-0" style={{ animationDelay: "250ms" }}>
                  {service.description}
                </p>
                <ul className="space-y-3 mb-8">
                  {service.features.map((feature, i) => (
                    <li
                      key={i}
                      className="flex items-start gap-3 text-muted-foreground animate-fade-up opacity-0"
                ...
                <Button variant="glass" size="lg" asChild className="animate-fade-up opacity-0" style={{ animationDelay: "600ms" }}>
                  <Link to="/contact" className="gap-2">
                    Get Started <ArrowRight className="w-4 h-4" />
                  </Link>
                </Button>
              </div>
              <div className={`${index % 2 === 1 ? "lg:order-1" : ""} animate-fade-up opacity-0`} style={{ animationDelay: "200ms" }}>
